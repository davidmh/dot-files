...
local function _28_()
  return vim.diagnostic.jump(core.merge({count = -1}, error_filter), opts("next diagnostic"))
end
vim.keymap.set("n", "[d", _28_)
local function _29_()
  return vim.diagnostic.jump(core.merge({count = 1}, error_filter), opts("previous diagnostic"))
end
vim.keymap.set("n", "]d", _29_)
local function _30_()
  return vim.diagnostic.jump(core.merge({count = -1}, warning_filter), opts("next warning"))
end
vim.keymap.set("n", "[w", _30_)
local function _31_()
  return vim.diagnostic.jump(core.merge({count = 1}, warning_filter), opts("previous warning"))
end
vim.keymap.set("n", "]w", _31_)
