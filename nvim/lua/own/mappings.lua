...
vim.keymap.set("n", "<localleader>no", _17_, opts("open notifications"))
local function _18_()
  return notifications.discard()
end
vim.keymap.set("n", "<localleader>nd", _18_, opts("dismiss notifications"))
local function _19_()
  return projects["select-project"]()
end
vim.keymap.set("n", "<localleader>p", _19_, opts("switch projects"))
vim.keymap.set("n", "Q", toggle_quickfix, opts("quickfix toggle"))
local function _20_()
  return snacks.picker.spelling()
end
vim.keymap.set("n", "z=", _20_, opts("suggest spelling"))
local function _21_()
  return vim.diagnostic.jump(core.merge({float = true, count = -1}, error_filter))
end
vim.keymap.set("n", "[d", _21_, opts("next diagnostic"))
local function _22_()
  return vim.diagnostic.jump(core.merge({float = true, count = 1}, error_filter))
end
vim.keymap.set("n", "]d", _22_, opts("previous diagnostic"))
local function _23_()
  return vim.diagnostic.jump(core.merge({float = true, count = -1}, warning_filter))
end
vim.keymap.set("n", "[w", _23_, opts("next warning"))
local function _24_()
  return vim.diagnostic.jump(core.merge({float = true, count = 1}, warning_filter))
end
vim.keymap.set("n", "]w", _24_, opts("previous warning"))
...
local function lsp_mappings()
  ...
  buf_map("<leader>la", _27_, "lsp: code actions")
  local function _28_()
    return vim.lsp.buf.rename()
  end
  buf_map("<leader>lr", _28_, "lsp: rename")
  buf_map("<leader>lR", "<cmd>LspRestart<CR>", "lsp: restart")
  local function _29_()
    return snacks.picker.lsp_symbols()
  end
  buf_map("<leader>ls", _29_, "lsp: symbols")
  local function _30_()
    return vim.lsp.buf.code_action()
  end
