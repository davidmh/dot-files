...
quickfix_title = {container({provider = _30_}), condition = show_quickfix_title}
...
lsp_breadcrumb = {container({provider = _31_}), condition = _32_, update = {"CursorMoved", "ColorScheme"}, hl = {fg = "fg"}}
...
local function diagnostic(severity_code, color)
  ...
  return {provider = _33_, condition = _34_, hl = {fg = color}}
end
...
diagnostics_block = {diagnostic("ERROR", "red"), diagnostic("WARN", "yellow"), diagnostic("INFO", "fg"), diagnostic("HINT", "green"), {provider = " "}, conditon = _35_, init = _36_, update = {"DiagnosticChanged", "BufEnter", "ColorScheme"}}
...
git_block = {container({{provider = _38_}, {provider = _39_, condition = has_git_diff("added")}, {provider = _40_, condition = has_git_diff("changed")}, {provider = _41_, condition = has_git_diff("removed")}}), condition = conditions.is_git_repo, init = _42_, hl = {bg = chrome_accent}}
local git_blame
local function _43_()
  return (vim.o.filetype == "fugitiveblame")
end
git_blame = {container({{provider = "git blame", hl = {bold = true}}}), condition = _43_}
local term_title
...
term_title = {{container({{provider = _44_}, {provider = " \238\170\182 "}, {provider = _45_}})}, condition = _46_, init = _47_}
local line_number
local function _48_()
  return vim.o.number
end
line_number = {provider = " %2{&nu ? (&rnu && v:relnum ? v:relnum : v:lnum) : ''} ", condition = _48_}
local fold
local function _49_()
...
local function initialize_heirline()
  ...
  opts = {colors = palettes.get_palette(), disable_winbar_cb = _52_}
