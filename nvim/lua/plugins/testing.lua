...
local function config()
  ...
  return neotest.setup({adapters = {neotest_go, neotest_python, neotest_rspec, neotest_playwright.adapter({options = {preset = "headed", persist_project_selection = true}})}, consumers = {playwright = neotest_playwright_consumers.consumers}, discovery = {enabled = false}, benchmark = {enabled = true}, icons = {failed = "\239\145\167", passed = "\239\144\174", running = "\239\145\170", watching = "\239\145\129"}})
end
return {"nvim-neotest/neotest", dependencies = {"nvim-lua/plenary.nvim", "antoinemadec/FixCursorHold.nvim", "nvim-treesitter/nvim-treesitter", "nvim-neotest/neotest-go", "nvim-neotest/neotest-python", "thenbe/neotest-playwright", "olimorris/neotest-rspec"}, keys = {{"<localleader>ta", "<cmd>Neotest attach<cr>", desc = "attach"}, {"<localleader>trf", "<cmd>Neotest run file<cr>", desc = "run file"}, {"<localleader>trl", "<cmd>Neotest run<cr>", desc = "run current line"}, {"<localleader>ts", "<cmd>Neotest summary<cr>", desc = "summary"}, {"<localleader>to", "<cmd>Neotest output<cr>", desc = "output"}, {"<localleader>tp", "<cmd>Neotest output-panel<cr>", desc = "panel"}, {"]t", "<cmd>Neotest jump next<cr>", desc = "next"}, {"[t", "<cmd>Neotest jump previous<cr>", desc = "previous"}}, config = config}
