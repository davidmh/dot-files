...
local function config()
  ...
  base_opts = {mapping = cmp.mapping.preset.insert(cmd_mappings), sources = cmp.config.sources({{name = "luasnip"}, {name = "nvim_lsp"}, {name = "async_path"}, {name = "emoji"}, {name = "nerdfonts"}, {name = "conjure"}, {name = "buffer", keyword_length = 5}}), formatting = {fields = {"icon", "abbr", "menu"}, format = cmp_format}, snippet = {expand = _3_}, window = {completion = {winhighlight = "Normal:Pmenu,FloatBorder:Pmenu,Search:None", col_offset = -3, border = "none", side_padding = 0}}}
  cmp.setup(base_opts)
  ...
  return ls.add_snippets("gitcommit", {co_authored_by})
end
return {{"hrsh7th/nvim-cmp", dependencies = {"hrsh7th/cmp-nvim-lsp", "hrsh7th/cmp-buffer", "PaterJason/cmp-conjure", "saadparwaiz1/cmp_luasnip", "L3MON4D3/LuaSnip", "davidmh/cmp-nerdfonts", "davidmh/cmp-git-co-authors", "onsails/lspkind-nvim", "hrsh7th/cmp-emoji", "kristijanhusak/vim-dadbod-completion", "https://codeberg.org/FelipeLema/cmp-async-path.git"}, event = "InsertEnter", config = config}}
